...
$album_per_page = 8;
$page_album = isset($_GET['page_album']) ? (int)$_GET['page_album'] : 1;
$start_album = ($page_album - 1) * $album_per_page;

$query_total_albums = mysqli_query($koneksi, "SELECT COUNT(*) as total FROM album WHERE user_id = '$user_id'");
$total_albums_result = mysqli_fetch_assoc($query_total_albums);
$total_albums = $total_albums_result['total'];

$total_pages_album = ceil($total_albums / $album_per_page);

$foto_per_page = 8;
$page_foto = isset($_GET['page_foto']) ? (int)$_GET['page_foto'] : 1;
$start_foto = ($page_foto - 1) * $foto_per_page;

$query_total_fotos = mysqli_query($koneksi, "SELECT COUNT(*) as total FROM foto WHERE user_id = '$user_id'");
$total_fotos_result = mysqli_fetch_assoc($query_total_fotos);
$total_fotos = $total_fotos_result['total'];

$total_pages_foto = ceil($total_fotos / $foto_per_page);
...
<div class="container mt-3 mb-5">
    ...
    <div class="text-light mx-4">
        <h4>Album Anda : </h4>
    </div>

    <div class="row">
        <?php
        $query_album = mysqli_query($koneksi, "SELECT * FROM album WHERE user_id='$user_id' LIMIT $start_album, $album_per_page ");
        if (mysqli_num_rows($query_album) == 0) {
            echo "<p class='text-center text-light'>Data Belum Ada</p>";
        } else {
            while ($album = mysqli_fetch_array($query_album)) {
                $query_foto = mysqli_query($koneksi, "SELECT * FROM foto WHERE album_id = " . $album['album_id'] . " ORDER BY foto_id ASC LIMIT 1");
                $foto = mysqli_fetch_array($query_foto);
                ?>

                <div class="col-md-3 mt-5">
                    <div class="card mx-4 bg">
                        <a href="album_detail.php?id=<?php echo $album['album_id']; ?>">
                            <img src="../assets/img/<?php echo $foto['lokasi_file']; ?>" class="card-img-top" alt=""
                                style="height: 10rem;" title="<?php echo $album['nama_album']; ?>">
                        </a>
                        <div class="card-body">
                            <h5 class="card-title truncate"><?php echo $album['nama_album']; ?></h5>
                            <a href="album_detail.php?id=<?php echo $album['album_id']; ?>" class="btn btn-secondary">Lihat
                                Album</a>
                        </div>
                    </div>
                </div>
    ...
    <div class="d-flex justify-content-center mt-4">
        <ul class="pagination mb-5">
            <?php if ($page_album > 1): ?>
                <li class="page-item ">
                    <a class="page-link" href="?page_album=<?php echo $page_album - 1; ?>">Previous</a>
                </li>
            <?php else: ?>
            ...
            <?php for ($i = 1; $i <= $total_pages_album; $i++): ?>
                <li class="page-item <?php echo ($i == $page_album) ? 'active' : ''; ?>">
                    <a class="page-link" href="?page_album=<?php echo $i; ?>"><?php echo $i; ?></a>
                </li>
            <?php endfor; ?>

            <?php if ($page_album < $total_pages_album): ?>
                <li class="page-item">
                    <a class="page-link" href="?page_album=<?php echo $page_album + 1; ?>">Next</a>
                </li>
            <?php else: ?>
                <li class="page-item disabled">
...
<div class="container mt-3 mb-5">
    ...
    <div class="row">
        ...
        WHERE foto.user_id = '$user_id' LIMIT $start_foto, $foto_per_page");
        
        if (mysqli_num_rows($devi_query_foto) == 0) {
            echo "<p class='text-center text-light'>Data Belum Ada</p>";
        } else {
                                                ...
                                                <form action="../c/aksi_komentar.php" method="POST">
                                                    <div class="input-group">
                                                        ...
                                                        <input type="text" name="isi_komentar" class="form-control"
                                                            placeholder="Tambah Komentar" required>
    ...
    <div class="d-flex justify-content-center mt-4">
        <ul class="pagination mb-5">
            <?php if ($page_foto > 1): ?>
                <li class="page-item">
                    <a class="page-link" href="?page_foto=<?php echo $page_foto - 1; ?>">Previous</a>
                </li>
            <?php else: ?>
                <li class="page-item disabled">
                    <span class="page-link">Previous</span>
            ...
            <?php for ($i = 1; $i <= $total_pages_foto; $i++): ?>
                <li class="page-item <?php echo ($i == $page_foto) ? 'active' : ''; ?>">
                    <a class="page-link" href="?page_foto=<?php echo $i; ?>"><?php echo $i; ?></a>
                </li>
            <?php endfor; ?>

            <?php if ($page_foto < $total_pages_foto): ?>
                <li class="page-item">
                    <a class="page-link" href="?page_foto=<?php echo $page_foto + 1; ?>">Next</a>
                </li>
            <?php else: ?>
                <li class="page-item disabled">
                    <span class="page-link">Next</span>

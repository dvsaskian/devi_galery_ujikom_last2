...
function time_ago($timestamp) {
    ...
    // Menghitung waktu dalam detik, menit, jam, hari, minggu, bulan, tahun
    ...
    // Menentukan tampilan waktu yang sesuai
    if ($seconds <= 60) {
        ...
        <div class="row mb-2">
            <div class="col-6">
                <div class="d-flex justify-content-start">
                    <a href="notifikasi.php" class="btn btn-primary mb-3"  data-toggle='tooltip' title='Bersihkan Notifikasi'> Tandai Semua Telah Dibaca</a>
                </div>
            </div>
            <div class="col-6">
                <div class="d-flex justify-content-end">
                    <a href="notifikasi_read.php" class="btn btn-success mb-3 ms-3"  data-toggle='tooltip' title='Lihat Notifikasi'>
        ...
        <?php if (mysqli_num_rows($notif_query) > 0) { ?>
            <div class="list-group">
                <?php while ($notif = mysqli_fetch_array($notif_query)) { ?>
                    <div class="card notification-card mb-2 <?php echo ($notif['status'] == 'unread') ? 'bg-light' : ''; ?>">
                        <div class="card notification-card mb-2 <?php echo ($notif['status'] == 'unread') ? 'bg-light' : ''; ?>">
                        ...
                        <div class="card-body d-flex align-items-center">
                            ...
                            <div>
                                ...
                                <br><small class='text-muted'>
                                    <?php echo time_ago($notif['tanggal']); // Gunakan fungsi time_ago() ?>
                                </small>
                            </div>
                            <a href='foto_detail.php?foto_id=<?php echo $notif['foto_id']; ?>' class='btn btn-sm btn-primary ms-auto'>Lihat Foto</a>
                        </div>
